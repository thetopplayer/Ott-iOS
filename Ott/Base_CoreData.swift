...
class Base_CoreData: NSManagedObject, Uploadable, MKAnnotation {
    ...
    func ratingToText() -> String {
        ...
        return BaseObject.ratingToText(rating?.floatValue)
    }
    ...
    func ratingToColor() -> UIColor {
        ...
        return BaseObject.ratingToColor(rating?.floatValue)
    }
    ...
    func toDictionary() -> [String : String] {
        ...
        result[BaseObject.typeKey] = "BaseObject"  // make sure to override this in subclasses
        
        if let myComment = comment {
            result[BaseObject.commentKey] = myComment
        }
        
        if let myLat = latitude {
            result[BaseObject.latitudeKey] = "\(myLat)"
        }
        
        if let myLocationName = locationName {
            result[BaseObject.locationNameKey] = myLocationName
        }
        
        if let myLong = longitude {
            result[BaseObject.longitudeKey] = "\(myLong)"
        }
        
        if let myName = name {
            result[BaseObject.nameKey] = myName
        }
        
        if let myRating = rating {
            result[BaseObject.ratingKey] = "\(myRating)"
        }
